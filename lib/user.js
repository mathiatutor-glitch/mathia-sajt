...
//  Probni period: 15 minuta (po telefonu, na serveru). Vreme kreće od PRVOG pitanja.
...
const TRIAL_MIN = parseInt(process.env.TRIAL_MINUTES || "15", 10);    // 15 minuta besplatno
const SHIELDS_PER_MONTH = parseInt(process.env.STREAK_SHIELDS || "2", 10);
export const LIMITS = { TRIAL_MIN, SHIELDS_PER_MONTH };
...
// Admin/owner bypass — ti si u ADMIN_EMAILS → neograničen pristup svemu
const DEFAULT_ADMINS = ['marina.bulat@gmail.com'];   // vlasnik — uvek admin (uz ADMIN_EMAILS iz env-a)
export function isAdmin(identifier) {
  if (!identifier) return false;
  const admins = DEFAULT_ADMINS
    .concat((process.env.ADMIN_EMAILS || '').split(','))
    .map(e => e.trim().toLowerCase()).filter(Boolean);
  return admins.includes(String(identifier).toLowerCase());
}
...
// Vrati {subscribed, expired, trial:{active, minutesLeft}}  — limit je SAMO vreme (15 min)
export function computeTrial(u) {
  if (isSubscribed(u)) {
    return { subscribed: true, expired: false, trial: { active: false, minutesLeft: 0 } };
  }
  if (!u || !u.trialStartedAt) {
    return { subscribed: false, expired: false, trial: { active: true, minutesLeft: TRIAL_MIN } };
  }
  ...
  return { subscribed: false, expired, trial: { active: !expired, minutesLeft } };
}
...
export function publicProfile(u) {
  ...
  return {
    ...
    badges: u.badges,
    goal: u.goal,
    tema: u.tema || null,   // izabrana boja korice/planera (pamti se na svim uređajima)
    wm: maskPhone(u.phone),
  };
}

// Maskiran broj za vodeni žig na materijalima (poslednje 3 cifre — dovoljno da se uđe u trag, a ne otkriva ceo broj)
function maskPhone(p) {
  if (!p) return "MathIA · mathia.rs";
  const d = String(p).replace(/\D/g, "");
  return d.length >= 3 ? "MathIA · ···" + d.slice(-3) : "MathIA · mathia.rs";
}
