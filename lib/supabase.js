...
export async function aktivirajPretplatu({ email, paket, predmeti, sati, dani, tip }) {
  const sada = new Date();
  const { data: postojeca } = await db.from('pretplate')
    .select('*').eq('kupac_email', email).eq('paket', paket).maybeSingle();

  // Trajanje: dopuna (klon) = prosleđeni sati (npr. 48h); paket = prosleđeni dani
  // (mesečni 30, godišnji 365); ako ništa nije prosleđeno — podrazumevano 30 dana.
  const trajanjeMs = sati
    ? (Number(sati) * 60 * 60 * 1000)
    : (dani ? (Number(dani) * 24 * 60 * 60 * 1000) : (30 * 24 * 60 * 60 * 1000));
  const osnova = (postojeca && new Date(postojeca.istice) > sada) ? new Date(postojeca.istice) : sada;
  const istice = new Date(osnova.getTime() + trajanjeMs);

  const red = { predmeti, istice, status: 'aktivna', obavesten_o_isteku: false };
  if (tip) red.tip = tip;   // 'klon' = dopuna (samo klon, ne i skripte); podrazumevano 'pun'

  if (postojeca) {
    await db.from('pretplate').update(red).eq('id', postojeca.id);
  } else {
    await db.from('pretplate').insert(Object.assign({ kupac_email: email, paket, pocinje: sada }, red));
  }
  return istice;
}
