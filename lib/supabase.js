...
export async function aktivirajPretplatu({ email, paket, predmeti, sati, tip }) {
  const sada = new Date();
  const { data: postojeca } = await db.from('pretplate')
    .select('*').eq('kupac_email', email).eq('paket', paket).maybeSingle();

  // Paket = 30 dana; dopuna (klon) = koliko sati je prosleđeno (npr. 48h).
  const trajanjeMs = sati ? (Number(sati) * 60 * 60 * 1000) : (30 * 24 * 60 * 60 * 1000);
