...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      days.push(day); vkeys.push("hit:v:" + day);
    }
    const views = (await kvCmd(["MGET", ...vkeys])) || [];
    const total = await kvCmd(["GET", "hit:v:total"]);

    let uToday = 0, u7 = 0;
    try { uToday = (await kvCmd(["SCARD", "hit:s:" + days[days.length - 1]])) || 0; } catch (e) {}
    try {
      const last7 = days.slice(-7).map((d) => "hit:s:" + d);
